...
| Id | Gap | Where | XSLTForms reference |
|---|---|---|---|
| G-20 | **PHASES 1+2 DONE 2026-08-30** (of 3) — phase 2: every `<table>` → cell-based NSTableView (`XFTableModel` flattens rows × columns: thead → column titles, else the first row's control labels; `xf:repeat` under table/tbody/tfoot → one row per item, static + tfoot rows kept, colspan content in its first column; control-only cells get NSTextFieldCell / NSSecureTextFieldCell / NSPopUpButtonCell / NSButtonCell / switch / NSSliderCell, mixed cells are text; edits, popups, checkboxes and trigger buttons go through the processor; row selection ↔ repeat index). Not done: XSLTForms' `appearance="table|table-row|table-cell"` on group/repeat, nested tables inside cells (shown as text). Tests: testTableModelRowsFromRepeatAndFooter, testTableModelHeaderRowGivesTitlesAndMixedCellsAreText. Phase 1: engine: `XFHostNode` tree (block / inline / text / br / hr / table parts / svg / control) built from the body and from every group / case / repeat item, instantiating controls at any depth like XSLTForms' node.xsl copy; UI: XFFormView lays out from the tree — blocks stack, inline runs (text, b/i/code…, inline controls) wrap at the form width, fieldset → NSBox, h1–h6 → bold sizes, pre → monospace, ul/li bullets, br/hr; controls are inline `<span>`s like XSLTForms (group/repeat/switch/textarea/full selects are blocks); repeat/group labels only from a direct `xf:label` child. **phase 3 (SVG) DONE 2026-08-31** — own NSBezierPath renderer (the surveyed option): `XFAVT` ports avtparser.xsl/XsltForms_avt ({expr} attributes, {{ }} escapes, unmatched braces literal; kept as compiled literal+XPath pieces re-evaluated at render-tree build instead of per-attribute controls — the form view rebuilds host trees on refresh, observable behavior matches); `XFSVGDocument`/`XFSVGView` (XFSVG.h public, impl AppKit/XFSVGView.m) build a resolved render tree from the XFHostNode subtree (repeat items give their node+position() context, bound groups their node, outputs gathered into <text>) and draw with NSBezierPath/NSAffineTransform: path data incl. arcs (F.6.5 center parameterization, NanoSVG consulted), rect/circle/ellipse/line/polyline/polygon, g, transforms (translate/scale/rotate[cx,cy]/matrix/skew), fill/stroke + style="" (colors #rgb/#rrggbb/rgb()/named/none; url(#…) degrades to a neutral wash — no gradients/patterns), text (font-size/-weight, text-anchor, baseline y), width/height units px|mm|cm|in|pt|pc, viewBox scaling, viewport clip. piechart + flags render fully; gantt: its width bind calls a page-JavaScript function (`lastday()` in a <script>); page JS stays unsupported, but 2026-08-31 (same day, second pass) `XFXPath registerHostFunctionNamed:` gives hosts the native equivalent of that fallback — consulted after the built-ins — and both apps register lastday(), so gantt computes its real width and its pattern background paints. The second pass also added the paint servers (linearGradient/radialGradient via NSGradient, pattern tiling, both unit systems, gradientTransform), defs/use (id registry, cycle-guarded), and render-tree hit testing (XFSVGDocument nodeAtPoint:/frameOfElement:, XFFormView svgElementAtPoint:/layoutFrameOfSVGElement:) that the designer's element-centric overlay picks and drag-reorders SVG shapes through (XFHostEdit moves SVG-ns elements freely among SVG-ns parents). Tests: testSVGPaintServersUseAndHitTesting, testHostRegisteredXPathFunctions. Tests: testAVTTemplates, testSVGParsingPrimitives, testSVGRenderTreeWithRepeatAndAVT. Was: POSTPONED 2026-08-30: library survey — SVGKit (CALayer/QuartzCore), IJSVG (macOS-only CGContext/CALayer, 20k lines), macSVG (an editor rendering through WebKit), PocketSVG (1.7k lines ObjC++ on CGPath, shapes only, no `<text>`) — none is portable to GNUstep as-is; when resumed the options are an own NSBezierPath renderer, a PocketSVG port + text, or IJSVG on macOS only. Until then `<svg>` shows a `[SVG]` placeholder (gantt, piechart, flags). Tests: testGroupInstantiatesControlsInsideHostMarkup, testRepeatInstantiatesControlsInsideTableRows, testCaseInstantiatesControlsInsideHostMarkup, testHostTreeKeepsBodyMarkupAndSkipsHead. Was: Controls nested in host markup (`<p>`, `<td>`, `<div>`, `<svg>`) inside `xf:group` / `xf:repeat` / `xf:case` are dropped (only direct `xf:` children instantiated; top-level collector already recurses). Breaks 7 samples and all table-layout repeats | `XFGroup.m`, `XFRepeat.m`, `XFSwitch.m` | `node.xsl.xml`, `group.xsl.xml` |
